        ...
        <div class="archivo-wrapper">
            <div class="soca-container">
                ...
                <div style="margin: 2rem 0;">
                    ...
                    <div
                        style="overflow: auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                        <table style="width: 100%; border-collapse: collapse;">
                            <thead>
                                <tr style="background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                                    <th
                                        style="border: 1px solid #dee2e6; padding: 12px; text-align: left; font-weight: 600; color: white;">
                                        TÍTULO</th>
                                    <th
                                        style="border: 1px solid #dee2e6; padding: 12px; text-align: left; font-weight: 600; color: white;">
                                        TIPO</th>
                            ...
                            <tbody>
                                @forelse($files as $f)
                                    <tr style="border-bottom: 1px solid #dee2e6; transition: background 0.2s;">
                                        <td style="border: 1px solid #dee2e6; padding: 12px;">{{ $f['name'] }}</td>
                                        <td style="border: 1px solid #dee2e6; padding: 12px;">{{ $f['tipo'] }}</td>
                                        <td style="border: 1px solid #dee2e6; padding: 12px;">{{ $f['created_at'] }}</td>
                                        <td style="border: 1px solid #dee2e6; padding: 12px; white-space: nowrap;">
                                            <a href="{{ route('obligaciones.documentos.download', $obligacion->id_obligacion) }}?file={{ urlencode($f['path']) }}"
                                                class="obl-btn"
                                                style="background: #007bff; color: white; text-decoration: none; font-size: 0.85rem; padding: 0.5rem 1rem;">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4"
                                            style="border: 1px solid #dee2e6; padding: 12px; color: #999; text-align: center;">
                                            <i class="fas fa-inbox" style="margin-right: 0.5rem;"></i>Aún no hay archivos.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <form method="POST"
                    action="{{ route('coordinador.obligaciones.dictamen.update', $obligacion->id_obligacion) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="decision-section">
                        ...
                        <p class="decision-text">
                            ¿El proyecto cumple con lo estipulado por el técnico y está listo para enviarse al Director?
                        </p>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const validarUrl = `{{ route('coordinador.obligaciones.firma.validar', $obligacion->id_obligacion) }}`;
            ...
            const updateSaveButton = () => {
                ...
                // Si es aprobado, necesita firma. Si no, no.
                const canSave = hasDecision && (!requiresFirma || hasFirma);

                saveBtn.disabled = !canSave;

                // Mostrar botones según decisión
                ...
                // Puedes agregar lógica para devolver al coordinador si lo necesitas
            };
