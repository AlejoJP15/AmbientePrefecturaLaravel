...
@section('content')
    ...
    <div class="main-container">
        <div class="container-fluid py-4">
            <div class="bg-white rounded shadow-sm p-4">
                <h2 class="mb-3 text-center">Proyectos aprobados por el coordinador (pendientes de tu evaluación)</h2>
                    ...
                    <table id="obligacionesTable" class="table table-striped table-hover w-100">
                        ...
                        <tbody>
                            @forelse ($obligaciones as $obl)
                                <tr>
                                    <td>{{ $loop->iteration + ($obligaciones->currentPage() - 1) * $obligaciones->perPage() }}</td>
                                    <td>
                                    ...
                                    <td>{{ $obl->estado_obligacion ?? '—' }}</td>
                                </tr>
                            @empty
                                {{-- NO pongas filas con colspan aquí; DataTables no las soporta --}}
                            @endforelse
                        </tbody>
                    </table>
                </div>
            
                {{-- Mensaje fuera de la tabla cuando no hay datos --}}
                @if ($obligaciones->isEmpty())
                    <div class="alert alert-info mt-3 mb-0">
                        No hay proyectos pendientes de tu evaluación.
                    </div>
                @endif
    ...
    <style>
        .dataTables_wrapper .dataTables_filter {
            float: right; text-align: right; margin-bottom: 10px; margin-right: 80px;
        }
        .dataTables_wrapper .dataTables_filter input { margin-left: 0.5em; width: 200px; max-width: 100%; }
    ...
    <script>
        $(function () {
            const hasRows = {{ $obligaciones->isNotEmpty() ? 'true' : 'false' }};
            if (!hasRows) return;
